...
import io.gumga.viagem.application.service.FornecedorService;
...
@Transactional
public class FornecedorAPI extends GumgaAPI<Fornecedor, String> {

    @Autowired
    private GumgaTempFileService gumgaTempFileService;

    @Autowired
    public FornecedorAPI(GumgaService<Fornecedor, String> service) {
        super(service);
    }
    ...
    public RestResponse<Fornecedor> save(@RequestBody @Valid Fornecedor obj, BindingResult result) {
        if (obj.getFoto() != null) {
    ...
    public RestResponse<Fornecedor> update(String id, @RequestBody @Valid Fornecedor obj, BindingResult result) {
        if (obj.getFoto()!= null) {
